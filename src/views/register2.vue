  ...
  <div class="register">
    ...
    >
      ...
      <el-form-item label="短信验证码" prop="phoneCode">
        ...
        <el-col :span="6">
          <el-button type="warning" :disabled="isDisabled" @click="sending">( {{btnText}}{{isDisabled?'s':''}} )</el-button>
        </el-col>
      </el-form-item>
      ...
      <div class="register-dialog-mian">
        <div class="register-dialog-title">
          ...
          <div class="register-dialog-matter1">
            <h3>一、基本合作事项</h3>
            <p v-for="(item,index) in agreementData.one">({{index+1}})、{{item.title}}</p>
          </div>
          <!-- 二、双方的权利和义务 -->
          <div class="register-dialog-matter1">
            <h3>二、双方的权利和义务</h3>
            <p>（一）、甲方的权利和义务</p>
            <p v-for="(item,index) in agreementData.two.one">({{index+1}})、{{item.title}}</p>
            <p>（二）乙方的权利和义务</p>
            <p v-for="(item,index) in agreementData.two.two">({{index+1}})、{{item.title}}</p>
          </div>
          <!-- 三、保密 -->
          <div class="register-dialog-matter1">
            <h3>三、保密</h3>
            <p v-for="(item,index) in agreementData.three">({{index+1}})、{{item.title}}</p>
          </div>
          <!-- 四、协议的变更、补充及终止 -->
          <div class="register-dialog-matter1">
            <h3>四、协议的变更、补充及终止</h3>
            <p v-for="(item,index) in agreementData.four">({{index+1}})、{{item.title}}</p>
          </div>
...
<script>
import { register, companyPhoneCode } from "@/api/login";
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          register(this.ruleForm).then(xhrData => {
            // console.log(xhrData);
            if (xhrData.code === 200) {
