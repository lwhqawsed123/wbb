...
import {
  ...
  insNonOnlineProd,
  insPubGetNewApplyDate,
  insPubGetAllowProject,
  //   续保
  insPubRenewal,
  insPubRenewalList,
  insPubDelRenewalProject,
  insPubRenewalGetAllowProject,
  insPubRenewalPost,
  insRuleGetProduct
...
export default {
  ...
  methods: {
    ...
    // 获取可申报的项目
    getAllowProject() {
      insPubGetAllowProject(this.obj.id).then(xhrData => {
    ...
    getApplyData() {
      ...
      // 获取 内在某个时间段内投保
      insPubGetNewApplyDate(this.obj.id).then(xhrData => {
    ...
    search() {
      ...
      insPubRenewalList(page).then(xhrData => {
    ...
    preserve() {
      this.$refs["from"].validate(valid => {
        if (valid) {
          ...
          // console.log(arr);
          insPubRenewalPost(arr).then(xhrData => {
      ...
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          if (row.id == undefined) {
            ...
            insPubDelRenewalProject(row.id).then(xhrData => {
    ...
    add() {
      ...
      // 查询退保新增项目允许的选择的项目
      insPubRenewalGetAllowProject().then(xhrData => {
        //   this.nameData =
