...
<script>
import { filePolicy, insPub } from "@/api/information/information";
import $ from "jquery";
export default {
  name: "declare-upload",
  ...
  mounted() {
    // this.getOssToken();
    // console.log(this.obj.from, "保单信息");
    // console.log(this.propertyData, "物业公司信息");
    // console.log(this.obj.insureNum, "保单号");
  },
  methods: {
    // //获取上传通行证
    // getOssToken() {
    ...
    // 上一步
    goBack() {
      this.$emit("changePage2", "PRINTING");
    },
    httpRequest(file) {
      // console.log(file);
    },
    ...
    submission() {
      ...
      const insPubDetailList = [];
      this.obj.from.forEach((item, index) => {
        // console.log(item);
        insPubDetailList.push({
        ...
        if (item.swimmingPool == "有") {
          insPubDetailList[index].swimmingPool = "0";
        } else if (item.swimmingPool == "无") {
          insPubDetailList[index].swimmingPool = "1";
        }
      ...
      if (
        ...
        insPub(jsonData).then(xhrData => {
          ...
          if (xhrData.code === 200) {
            ...
            this.$emit("changePage2", "PAYMENT", this.obj);
          }
        });
      }
    },
