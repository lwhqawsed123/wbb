    ...
    <el-form id="paymentForm" ref="form" :rules="rules" :model="form" label-width="120px">
      <el-row>
        <el-col :span="10">
          ...
          <el-form-item v-if="!isDisabled"  prop="telphoneNo" label="电话号码:">
          ...
          <el-form-item v-if="!isDisabled" label="区域" prop="selectedOptions">
            ...
            <el-cascader v-model="form.selectedOptions" :options="options" @change="handleChange"></el-cascader>
          </el-form-item>
...
export default {
  ...
  mounted() {
    ...
    this.getInvoiceList();
    this.getRegion();
    this.applyId = this.arrData.id;
    this.totalPremium = this.arrData.totalPremium;
  },
  methods: {
    //获取省市区
    getRegion() {
    ...
    submission() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          // this.form.telphoneNo =
          //   this.form.phoneCode + "-" + this.form.telephone;
          insPubConfirmPay(this.form).then(xhrData => {
            if (xhrData.code === 200) {
              this.$message({
                message: "提交成功，请耐心等待，保险经办人会尽快处理的",
                type: "success"
              });
              this.$router.push({name:"PublicList"})
            }
          });
        } else {
          this.$message({
