        ...
        >
          ...
          <el-form-item>
            ...
            <el-button
              ...
              plain
              @click="handleExport"
              v-hasPermi="['ins:prop:record:export']"
            >导出</el-button>
            <el-button
              type="primary"
              v-if="Prohibit"
              @click="renewal"
              plain
              v-hasPermi="['ins:prop:renewal:add']"
            >续保</el-button>
          </el-form-item>
        </el-form>
      </el-col>
        ...
        <template slot-scope="scope">
          <div style="display: flex;">
            ...
            <el-button
              ...
              icon="el-icon-view"
              @click="see(scope.row)"
              v-hasPermi="['ins:prop:record:list']"
            >查看</el-button>
            <el-button
            ...
            <el-button
              ...
              v-hasPermi="['ins:prop:return:list']"
            >退保</el-button>
            <el-button
              v-if="scope.row.state == '在保'||scope.row.state == '已过期'"
              size="mini"
...
export default {
  ...
  created() {
    this.getList();
    // 查询是否可续保
    insPropAllowRenewal().then(xhrData=>{
      if(xhrData.code === 200){
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出数据项?", "警告", {
        ...
        .then(function() {
          return insPropExportInsPubApply(formInline);
        })
        .then(response => {
    ...
    confirm(row){
      ...
      this.$router.push({name:"PropertyDeclare",query:{insData}})
    },
    renewal() {
      // const idData = {
      //   id: row.id
