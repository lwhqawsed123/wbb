  ...
  <div class="page-content">
    ...
    >
      <el-form-item label="项目名称:">
        <el-input @keyup.enter.native="search" v-model.trim="form.name"></el-input>
      </el-form-item>
    ...
    <el-table :data="projectTableData" style="width: 100%">
      ...
      <el-table-column prop="projectName" label="项目名称" min-width="10%" show-overflow-tooltip></el-table-column>
      <el-table-column prop="address" label="项目地址" min-width="10%" show-overflow-tooltip></el-table-column>
      <el-table-column prop="type" label="小区类型" min-width="10%" show-overflow-tooltip></el-table-column>
      <el-table-column prop="insureNum" label="投保份数" min-width="6%" show-overflow-tooltip></el-table-column>
      <el-table-column prop="startTime" label="投保起始日期" min-width="10%" show-overflow-tooltip></el-table-column>
      <el-table-column prop="endTime" label="投保结束日期" min-width="10%" show-overflow-tooltip></el-table-column>
      <el-table-column prop="areaSize" label="项目占地面积(万平方米)" min-width="14%" show-overflow-tooltip></el-table-column>
      <el-table-column prop="swimmingPool" label="是否有游泳池" min-width="10%" show-overflow-tooltip></el-table-column>
      <el-table-column prop="premium" label="保费" min-width="10%" show-overflow-tooltip></el-table-column>
    ...
    >
      ...
      <el-form :model="fromData" ref="from">
        <div class="page-table">
          ...
          >
            ...
            <el-table-column show-overflow-tooltip prop="date" label="小区类型" width="150px">
              <template slot-scope="scope">{{scope.row.type}}</template>
            </el-table-column>
            <el-table-column prop="date" label="投保份数" width="100px">
              <template slot-scope="scope">
                <el-form-item
            ...
            <!-- <el-table-column prop="date" label="投保起始日期" min-width="18%">
              <template slot-scope="scope">
                <el-form-item
            ...
            <el-table-column prop="date" label="投保起始日期" width="200px">
              <template>
                <el-input :disabled="true" style="width:100%" v-model="startTime"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="date" label="投保结束日期" width="200px">
              <template>
                <el-input :disabled="true" style="width:100%" v-model="endTime"></el-input>
              </template>
            </el-table-column>
      ...
      <span slot="footer" class="dialog-footer">
        <a href="javascript:;" class="btn btn1" @click="preserve">保 存</a>
...
import {
  ...
  insNonOnlineProd,
  insPropGetNewApplyDate,
  insPropGetAllowProject,
  //   续保
  insPropRenewal,
  insPropRenewalList,
  insPropDelRenewalProject,
  insPropRenewalGetAllowProject,
  insPropRenewalPost,
  insRuleGetProduct
} from "@/api/information/information";
// 保险条款
import { publicClause,propertyRisk } from "@/validate/programme"
export default {
  name: "apply-page-addedDeclare",
  props: ["propertyData", "obj"],
  data() {
    return {
      additionalClause:'',
      insuranceClause:'',
  ...
  created() {
    this.additionalClause = propertyRisk()
    this.insuranceClause = publicClause()
    // this.getProjecData();
    // this.getMoney();
    // this.getApplyData();
    // this.getAllowProject();
    this.getPubRenewalList();
    this.getRuleGetProduct();
  },
  methods: {
    // 获取续保列表
    getPubRenewalList() {
      insPropRenewal().then(xhrData => {
        // console.log(xhrData, "续保");
        if (xhrData.code === 200) {
          this.projectTableData = xhrData.rows;
          this.startTime = this.projectTableData[0].startTime.split(" ")[0];
          this.endTime = this.projectTableData[0].endTime.split(" ")[0];
    ...
    // 获取可申报的项目
    getAllowProject() {
      insPropGetAllowProject(this.obj.id).then(xhrData => {
        // console.log(xhrData, "申报的项目");
        if (xhrData.code === 200) {
          this.nameData = xhrData.data;
        }
      });
    },
    // 获取公众责任险保费与保额
    ...
    //   });
    // },
    // 根据当前id 获取 对应信息(在保日期内)
    getApplyData() {
      // console.log(this.obj, "123");
      // 获取 内在某个时间段内投保
      insPropGetNewApplyDate(this.obj.id).then(xhrData => {
        // console.log(xhrData, "在保日期内");
        if (xhrData.code === 200) {
          // 投保开始时间
          this.fromData.startTime = xhrData.data.startTime.split(" ")[0];
          // 投保结束时间
          this.fromData.endTime = xhrData.data.endTime.split(" ")[0];
          let startTime = new Date(this.fromData.startTime);
          let endTime = new Date(this.fromData.endTime);
    ...
    validityTerm(val, index) {
      ...
      // console.log("天", this.DateDiff(val, this.fromData.endTime));
      // 保存 选择日期开始到结束之间的天数
      // const day = this.DateDiff(val, this.fromData.endTime);
      this.keepDay = this.DateDiff(val, this.fromData.endTime);
      ...
      return {
        disabledDate(date) {
          ...
          return (
            ...
            date.getTime() < _this.time - 8.64e7
          );
        }
      };
    },
    ...
    next() {
      if (this.projectTableData.length <= 0) {
        ...
        const obj = {
          from: this.projectTableData,
          insCompanyId:this.insCompanyId
        };
        this.$emit("changePage2", "PRINTING", obj);
      }
    },
    // 添加项目
    addProject() {
    ...
    search() {
      ...
      insPropRenewalList(page).then(xhrData => {
        // console.log(xhrData, "查询出来的项目");
        if (xhrData.code === 200) {
          this.projectTableData = xhrData.rows;
          this.totalNum = xhrData.total;
          this.projectTableData.forEach(item => {
        ...
        if (valid) {
          ...
          this.fromData.tableData.forEach(item => {
            ...
            arr.push({
              ...
              type: item.type == "居住类型" ? "0" : "1",
              swimmingPool: item.yc == "有" ? "0" : "1"
            });
          });

          // console.log(arr);
          insPropRenewalPost(arr).then(xhrData => {
            // console.log(xhrData, "新增续保项目");
            if (xhrData.code === 200) {
              this.$message({
                message: "添加成功",
                type: "success"
          ...
          //   this.$emit("changePage2", "ADDED_UPLOAD", obj);
        } else {
          this.$message({
            message: "请完善信息！",
            type: "warning"
      ...
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          if (row.id == undefined) {
            ...
            insPropDelRenewalProject(row.id).then(xhrData => {
              // console.log(xhrData, "删除");
              if (xhrData.code === 200) {
                this.$message({
                  type: "success",
                  message: "删除成功!"
    ...
    add() {
      ...
      // 查询退保新增项目允许的选择的项目
      insPropRenewalGetAllowProject().then(xhrData => {
        //   this.nameData =
        // console.log(xhrData, "查询退保新增项目允许的选择的项目");
        if (xhrData.code === 200) {
          this.nameData = xhrData.data;
        }
      });
