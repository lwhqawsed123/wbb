<template>
  <div class="added-addedUpload">
    ...
    <!-- <printf-template :formData="this.obj.form" style="display: none"></printf-template> -->
...
export default {
  ...
  methods: {
    // 获取选中后的保单号
    getApplyDetail() {
      insPropGetApplyDetail(this.obj.form.id).then(xhrData => {
        // console.log(xhrData);
        if (xhrData.code === 200) {
          this.obj.form.orderNo = xhrData.data.orderNo;
          // console.log(this.obj, "objData1");
    ...
    submission() {
      ...
      insPropReturnApply(upData).then(xhrData => {
        // console.log(xhrData);
        if (xhrData.code === 200) {
          this.$message({
