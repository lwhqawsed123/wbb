...
export default {
  ...
  methods: {
    // 获取失败返回数据
    getAgainData(){
      insEmpApplyManageNewAgain(this.obj2.newApplyId).then(xhrData=>{
        if(xhrData.code === 200){
          // console.log(xhrData);
          // this.form.insEmpApplyUsers = []
          let arr = []
          let obj = {
             // 被保险人
            name: "",
          ...
          let newObj = {}
         arr = xhrData.data.insEmpApplyDetailList
          arr.forEach((item,index)=>{
            // 1console.log(this.objFun(this.ruleForm.insEmpApplyUsers,item));
            newObj = this.objFun(obj,item)
            newObj.jobCodeItem.code = item.jobCode
            newObj.jobCodeItem.name = item.jobName
            newObj.projectItem.name = item.projectName
            newObj.projectItem.id = item.projectId
            newObj.projectItem.remark = item.projectType == '0' ? '物业用户':''
            this.$set(this.form.insEmpApplyUsers[index],'name',newObj.name)
            this.$set(this.form.insEmpApplyUsers[index],'idCard',newObj.idCard)
            this.$set(this.form.insEmpApplyUsers[index],'phone',newObj.phone)
            this.$set(this.form.insEmpApplyUsers[index],'projectId',item.projectId)
            this.$set(this.form.insEmpApplyUsers[index],'projectType',item.projectType)
            this.$set(this.form.insEmpApplyUsers[index],'jobCode',item.jobCode)
            this.$set(this.form.insEmpApplyUsers[index],'jobName',item.jobName)
            this.$set(this.form.insEmpApplyUsers[index],'jobCodeItem',newObj.jobCodeItem)
            this.$set(this.form.insEmpApplyUsers[index],'projectItem',newObj.projectItem)
          })
    ...
    submitForm(form) {
      ...
      const data = {
        ...
        newApplyId:this.obj2.newApplyId
      };
