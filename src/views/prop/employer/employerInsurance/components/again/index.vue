    ...
    <div class="declare-conter">
      ...
      >
        ...
        <div v-if="!isInsurance" v-show="isAdditional" class="type-box">
          <el-radio v-model="type1" :label="type1">伤残赔付特约（伤残赔付比例按等比赔付） / 年</el-radio>
        </div>
        <div v-if="!isInsurance" v-show="isAdditional" class="type-box">
          <el-radio v-model="type2" :label="type2">扩展24小时（死亡伤残10万意外医疗1万） / 年</el-radio>
        </div>
        <!-- 保障周期 -->
        <div style="padding-bottom:20px">
          ...
          <el-radio-group @change="getSelectionType" v-model="selectionType.label">
            ...
            <el-radio :disabled="isShowYear" class="declare-radio" label="Y">
            ...
            <el-radio :disabled="isShowMonth" class="declare-radio" label="M">
            ...
            <el-radio :disabled="isShowDay2" class="declare-radio" label="D">
        ...
        <div>
          ...
          <div class="batchData">
            ...
            <el-form :model="formTable" ref="formTable">
              <el-table
                max-height="500px"
                :data="formTable.formdataArr.slice((currentPage-1)*pagesize,currentPage*pagesize)"
                border
                style="width: 1151px"
              >
                ...
                <el-table-column label="项目名称" width="300px">
                  <template slot-scope="scope">
                    <el-form-item
                      ...
                      :rules="formTable.rulesTable.project"
                    >
                      <el-select
                        style="width:100%"
                ...
                <el-table-column label="雇员姓名" width="100px">
                  <template slot-scope="scope">
                    <el-form-item
                      :prop="'formdataArr.' + scope.$index + '.name'"
                      :rules="formTable.rulesTable.name"
                    >
                ...
                <el-table-column label="身份证" width="200px">
                  <template slot-scope="scope">
                    <el-form-item
                      ...
                      :rules="formTable.rulesTable.idCard"
                    >
                ...
                <el-table-column label="手机号" width="200px">
                  <template slot-scope="scope">
                    <el-form-item
                      ...
                      :rules="formTable.rulesTable.phone"
                    >
                      <el-input maxlength="11" v-model.trim="scope.row.phone"></el-input>
                    </el-form-item>
                  </template>
                </el-table-column>
                <el-table-column prop="jobCode" label="工种" min-width="200px">
                  <template slot-scope="scope">
                    <el-form-item
                      :prop="'formdataArr.' + scope.$index + '.jobCodeItem'"
                      :rules="formTable.rulesTable.jobCodeItem"
                    >
                      <el-select
                        value-key="name"
        ...
        <div class="declare-btn-wrap">
          <!-- 点击批量申报时候出现的按钮 -->
          <el-button class="declare-btn btn1" @click="batchApplyDeclare">申报</el-button>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.formTable.formdataArr = data.insEmpApplyDetailList;
      // console.log(this.formTable.formdataArr);
      
      // if (this.formTable.formdataArr.length) {
      //   // 关闭对话框
      //   loading.close();
      // }
      this.total = this.formTable.formdataArr.length;

      this.formTable.formdataArr.forEach((item, index) => {
        arr1.push({
        ...
        this.$set(this.formTable.formdataArr[index], "project", arr1[index]);
        this.$set(
          this.formTable.formdataArr[index],
          "jobCodeItem",
          arr2[index]
        );
      });
      if (data.cycle == "Y") {
        ...
        this.selectionType.label = "Y";
      } else if (data.cycle == "M") {
        this.isShowMonth = false;
        this.selectionType.label = "M";
      } else if (data.cycle == "D") {
        this.isShowDay2 = false;
        this.selectionType.label = "D";
        this.isShowInvoice = false;
        this.ruleForm.code = "PLAN_THREE";
        this.isSocialSecurity2 = false;
      }
      this.showMoney(data.planCode);
    },
    // 获取工种信息
    ...
    handleChange(file, fileList) {
      ...
      insEmpApplyImport(param)
        .then(xhrData => {
          ...
          let arr2 = [];
          if (xhrData.code === 200) {
            this.formTable.formdataArr = xhrData.data;
            this.total = this.formTable.formdataArr.length;
            this.formTable.formdataArr.forEach((item, index) => {
              arr1.push({
                name: item.projectName,
    ...
    batchApplyDeclare() {
      ...
      if (this.formTable.formdataArr.length <= 0) {
        this.$message.warning({
          message: "请上传文件"
        });
        return;
      ...
      this.formTable.formdataArr.forEach(item => {
    ...
    optionArr(val, index) {
      if (val.remark == "物业用户") {
        this.formTable.formdataArr[index].projectType = "0";
      } else {
        this.formTable.formdataArr[index].projectType = "1";
      }
      this.formTable.formdataArr[index].compProjectName = val.name;
      this.formTable.formdataArr[index].compProjectId = val.id;
    },
    //
    getJobCode2(val, index) {
      this.formTable.formdataArr[index].jobFormdataCode = val.code;
    },
