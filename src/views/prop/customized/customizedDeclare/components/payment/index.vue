    ...
    <el-form id="paymentForm" ref="form" :rules="rules" :model="form" label-width="120px">
      <el-row>
        <el-col :span="10">
          ...
          <el-form-item v-if="!isDisabled" label="电话号码:">
            <el-col :span="7">
              <el-form-item prop="phoneCode">
                <el-input oninput="value=value.replace(/[^\d]/g,'')" @input="change($event)" :maxlength="5" v-model.trim="form.phoneCode"></el-input>
              </el-form-item>
            </el-col>
            <el-col class="line" :span="1" style="text-align: center">-</el-col>
            <el-col :span="16">
              <el-form-item style="display: inline-block;width:100%" prop="telephone">
                <el-input oninput="value=value.replace(/[^\d]/g,'')" @input="change($event)" :maxlength="8" v-model.trim="form.telephone"></el-input>
              </el-form-item>
            </el-col>
          </el-form-item>
...
export default {
  ...
  mounted() {
    ...
    this.totalPremium = this.obj.totalPremium + '元'
  ...
  methods: {
    ...
    getInsCompany() {
      ...
      // 根据id 查询保险信息
      insCompanyId(this.obj.form.company).then(xhrData => {
        // console.log(xhrData);
        if (xhrData.code === 200) {
          this.insCompany = xhrData.data;
        }
      });
    },
    // 获取开票抬头信息
    ...
    handleChange() {
      ...
      if (this.form.provinceCode !== undefined) {
        // 省 名
        this.form.provinceName = CodeToText[this.form.provinceCode];
      } else {
        // 省 名
      ...
      if (this.form.areaCode !== undefined) {
        // 区名
        this.form.areaName = CodeToText[this.form.areaCode];
      } else {
        // 区名
        this.form.areaName = "";
      }
    ...
    submission() {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          const data = {
            id:this.obj.id,
            invoiceId:this.titleId
          }
           //   alert("submit!");
          this.form.applyId = data.id;
          this.form.insCompId = this.insCompany.id;
          this.form.id = data.invoiceId
          this.form.telphoneNo = this.form.phoneCode+'-'+this.form.telephone
          // console.log(this.form);
          
          // 请求
          insEmpCustomApplyPut(this.form).then(xhrData => {
            if (xhrData.code === 200) {
              this.$message({
              ...
              this.$router.push({name:"CustomizedInsurance"})
