<template>
  <div>
      ...
      <el-col :span="24">
        ...
        >
          ...
          <el-form-item>
            ...
            <!-- ins:emp:custom:manage:return:add -->
            <el-button v-if="obj.state =='在保'" type="primary" plain @click="replaceBtn" v-hasPermi="['ins:emp:custom:manage:return:add']">退保</el-button>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="phone" show-overflow-tooltip label="雇员手机号" min-width="150px"></el-table-column>
    ...
    ></el-pagination>
    <!-- 审核 对话框 -->
    <el-dialog @close="closeDialog" :title="titleName" class="seeDialog" :visible.sync="dialogVisible" width="600px">
...
<script>
import {
  insEmpCustomApplyManageReturnList,
  insEmpCustomApplyManageDetailList,
  insEmpCustomApplyManageReturn
...
export default {
  ...
  created() {
    ...
    this.getList();
    // console.log(this.$store.state.user.userType,'用户类型');
  },
  mounted() {},
  methods: {
    // 获取 姓名信息
    ...
    getList() {
      ...
      // console.log(page);
      insEmpCustomApplyManageReturnList(page).then(xhrData => {
        // console.log(xhrData);
        if (xhrData.code === 200) {
    ...
    surrender() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          insEmpCustomApplyManageReturn(data).then(xhrData => {
            if (xhrData.code === 200) {
              this.$message({
                message: "退保成功！",
                type: "success"
