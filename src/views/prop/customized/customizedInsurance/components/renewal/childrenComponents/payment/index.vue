      ...
      <el-row>
        <el-col :span="10">
          ...
          <el-form-item label="开票抬头:" prop="invoiceId">
            <el-select @change="rise" v-model="form.invoiceId" placeholder="请选择开票抬头">
              <el-option
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        einNo: [{ required: true, message: "请输入", trigger: "blur" }],
        ...
        recipient: [{ required: true, message: "请输入", trigger: "blur" }],
        // 手机号
        mobileNo: [
  ...
  mounted() {
    ...
    this.totalPremium = this.obj2.totalPremium + '元'
  ...
  methods: {
    ...
    getInsCompany() {
      ...
      // 根据id 查询保险信息
      insCompanyId(this.obj2.form.company).then(xhrData => {
        // console.log(xhrData);
        if (xhrData.code === 200) {
    ...
    submission() {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          // console.log(data);
          // 请求
          insEmpCustomApplyPut(data).then(xhrData => {
            if (xhrData.code === 200) {
              this.$message({
