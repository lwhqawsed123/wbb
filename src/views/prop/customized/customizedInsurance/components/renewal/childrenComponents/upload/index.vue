...
<script>
import { filePolicy, insEmpCustomApply } from "@/api/information/information";
import $ from "jquery";
export default {
  name: "declare-upload",
  props: ["obj2"],
  ...
  methods: {
    ...
    // 提交
    submission() {
      // console.log(this.obj2.form);
      // this.$emit("changePage", "PAYMENT", this.obj);
      const empCustomDetailList = [];
      this.obj2.form.insEmpApplyUsers.forEach((item, index) => {
        ...
        if (item.isSocialSecurity == "无") {
          empCustomDetailList[index].isSocialSecurity = "0";
        } else if (item.isSocialSecurity == "有") {
          empCustomDetailList[index].isSocialSecurity = "1";
        }
      });
      const jsonData = {
        empCustomDetailList,
        ...
        // 周期
        cycle: this.obj2.form.cycle,
        // 开始时间
        startTime: this.obj2.form.startTime,
        // 保险公司id
        insCompId: this.obj2.form.company,
        // 方案id
        planId: this.obj2.form.code,
        // 凭证号
        orderNo: this.obj2.orderId
      };
      if (
        ...
        // 请求 提交
        insEmpCustomApply(jsonData).then(xhrData => {
          // console.log(xhrData);
          if (xhrData.code === 200) {
            this.$message({
              message: "提交成功！",
            ...
            this.obj2.id = xhrData.data.id;
            this.obj2.totalPremium = xhrData.data.totalPremium;
            this.$emit("changePage2", "PAYMENT", this.obj2);
          }
        });
      }
    },
